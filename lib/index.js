...
const mkHelpers = (salt) => {
  ...
  return chanceFunctionNames.reduce((helpers, key) => {
    helpers[key] = (...args) => {
      ...
      calcMakingValue(args, hashes, `helper[${key}] arguments `);

      return setHashAndReturn(() => chance[key](...args), `chance.${key}(${hashObj(hashes)},${salt})`);
    };
...
const Schema = function Schema (definitionFn, { limit = 10, salt = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' } = {}) {
  ...
  const generator = setHashAndReturn(() => scanningGenerate(value), hash);

  generator.version = () => hash;

  generator.limit = Math.abs(limit || 1);
