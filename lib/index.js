...
const mkHelpers = (salt, additional = {}) => {
  const chance = new Chance(salt);

  let helpers = Object.keys(additional).reduce((helpers, key) => {
    helpers[key] = wrapFn(additional[key].bind(additional), `additional.${key}`);

    return helpers;
  }, {});

  helpers = chanceFunctionNames.reduce((helpers, key) => {
...
const Schema = function Schema (definitionFn, { salt = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', customHelpers = {} } = {}) {
  salt = sha1(salt).toString();

  const definition = definitionFn(mkHelpers(salt, customHelpers));
  ...
  const generator = setHashAndReturn(() => setHashAndReturn(() => generateOne(), hash), hash);
